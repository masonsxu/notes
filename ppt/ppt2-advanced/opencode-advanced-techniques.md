...
| **前置要求** | 入门篇的原理（agent 循环）、Plan / Build、AGENTS.md、权限确认、三件套环境（opencode / VS Code / Git） |
...
> 这张图在说什么：左右两侧各有三个并列的病因 / 资产——它们是并列关系，不是先后步骤；各自汇聚出共同的后果 / 效果。差距不在工具版本，在这两组之间。
...
模型换代、上下文窗口变长、工具变强——每次升级，同一批记录的产出自动变好。所以**记录本身就是资产**——越早开始记，复利时间越长。这也是"从今天开始"的理由。
...
| ⑤ | 日常运转 | 每天用 VS Code 花几分钟写 `daily/` 记录（边写边预览）；周五一句话生成周报 |
...
> 这张图在说什么：同一批输入走两条路——人工路径的工作量随文件数线性增长，口径变更等于全部重做；skill 路径把工作量压成一句话，口径变更只是一次剧本修改。第三部分要做的，就是把这项工作从人工路径搬到 skill 路径。
...
3. **人工调整**：补口径、边界、红线——**五步里唯一需要你的专业判断的步骤**，后续产出质量取决于这一步做得多细。
4. **沉淀 skill**：让 agent 把这次跑通的工作流写成 SKILL.md（见 3.4）。
5. **一句话调用**：之后每次新任务就是"跑一遍 batch-report skill"，agent 在循环里批量、自动地完成，你用 diff 核对它动的每个文件（VS Code 红绿对照），再审最终汇总表。
...
首次投入约半天；从此每周的工作从一整天变成"一句话 + diff 核对改动 + 审核一份汇总表"。
...
| 不敢放开让 agent 批量执行 | 缺 Git 安全网（存档 / 回退） | 入门篇 3.6 三件套 |
