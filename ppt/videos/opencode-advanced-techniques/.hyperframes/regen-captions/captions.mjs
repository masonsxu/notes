...
// ── grouping params ───────────────────────────────────────────────────────────
const SILENCE_GAP = 0.25; // s of silence between words → split
const TAIL_PAD = 0.12; // s the group lingers after its last word
// Hard sentence end (CJK + ASCII) → always flush: the complete-subtitle unit.
const SENT_END = /[。！？!?…]$/;
// Soft clause break (CJK + ASCII) → preferred cut point when a line gets long.
const CLAUSE_END = /[，、；：,;:—]$/;
// Max chars per group before a soft split at the last clause break (keeps a
// line within the pill's 80% width at ~54px CJK glyphs ≈ 28 chars; 18 leaves
// padding + one-word wiggle so it wraps to at most 2 lines).
const MAX_CHARS = 18;
...
function runBuild(argv) {
  ...
  // group: split on frame change / silence gap / char budget (at a clause
  // break); always flush after a sentence-ending word — the complete-subtitle
  // unit. Works for CJK (1 glyph = 1 token) and Latin alike: no density word-cap
  // (that shrank CJK lines to 2 glyphs), a char budget instead.
  const groups = [];
  let cur = null;
  for (let i = 0; i < words.length; i++) {
    ...
    if (!cur || crossFrame || gap) {
      if (cur) groups.push(cur);
      cur = { frame: w.frame, words: [] };
    }

    // char budget: adding w would exceed MAX_CHARS → cut the group at the last
    // clause break (soft), carrying the overflow into a new group; no clause
    // break at all → hard split (a single long run-on without any punctuation).
    if (cur.words.length) {
      const curChars = cur.words.reduce((n, x) => n + x.text.length, 0);
      if (curChars + w.text.length > MAX_CHARS) {
        const clauseIdx = cur.words.findLastIndex((x) => CLAUSE_END.test(x.text));
        if (clauseIdx >= 0) {
          const overflow = cur.words.splice(clauseIdx + 1);
          if (cur.words.length) groups.push(cur);
          cur = { frame: w.frame, words: overflow };
        } else {
          groups.push(cur);
          cur = { frame: w.frame, words: [] };
        }
