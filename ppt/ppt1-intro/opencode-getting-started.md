...
> 这张图在说什么：同一个任务，两条路——聊天应用走到"给你一段文字"就停了，剩下的是你的事；opencode 走完读文件、统计、写成成品的全部步骤。虚线是接力点：聊天应用停下的地方，agent 从这里接手。颜色约定全文通用：琥珀 = 人做的事，蓝色 = agent 做的事，灰色 = 文件产物。
...
> 这张图在说什么：左边是聊天应用的组成——只有大脑，没有手、不循环，所以只会"说"；右边是 agent 的组成——大脑、手、循环三件并列。虚线就是本节标题说的那次跃迁——接上手、加上循环。
...
> 这张图在说什么：你的一句指令，在 agent 内部展开成多轮"思考 → 行动 → 观察"的循环。它每调一次工具、看一次结果，再决定下一步；任务完成后把成品写进工作目录。末尾的横条注记是关键安全设计：涉及写文件、执行命令的动作，先经你确认。
...
> 这张图在说什么：Plan 和 Build 不是两个功能，是同一个循环的两种配置。Plan 关掉"行动"环节，只让它思考和说明方案——零风险；Build 放开"行动"，循环开始真正干活。菱形是你要把关的决策点；中间绿色的确认门跨过之后，循环才真正动手。
...
> **本节回答**：怎么走完企业安装流程？界面里真正需要认识的只有哪三样？第一条指令怎么下，才能安全拿到交付物？五分钟跑通之后，为什么还要配齐 VS Code 和 Git？
...
先切换到 **Plan 模式**（右下角显示 `plan`）——此模式下它只说方案、不动文件（2.5 的 plan 循环）。按你的岗位选一个场景：
...
| 用途 | 说明 |
| --- | --- |
| 写 Markdown | 工作记录、报告、模板都是 Markdown，边写边预览（`Cmd+Shift+V`） |
| 看改动（diff） | agent 动了哪些文件、改了哪几行，红绿对照一目了然——审核的核心手段 |
| 浏览项目 | 按目录树浏览整个工作目录，文件再多不乱 |
| opencode 侧边栏 | 装上 IDE 插件，agent 直接嵌在编辑器里（2.2 三形态表的第三种） |

> VS Code 官网下载安装即可，企业环境优先走内部商店。这两份教学文档的现场演示，用的就是 VS Code 的 Markdown 预览。

**Git：放手让 agent 干活的安全网**

一句话理解：**Git 给文件夹装上"存档点"**——任何时候都能看清"改了什么"（diff）、回到"之前的状态"（回退）。它是 5.3 里 `/undo` 的前提，是你审核 agent 改动的依据，也是进阶篇里团队共享资产的载体。
...
> Git 本身通常由 IT 统一安装（Mac 多数已自带），没有就走内部商店装一次。习惯从下一个项目开始养成：**打开目录的第一句话，先让 agent 初始化 Git**——安全网要在任务之前存在。

> **带走一句话**：委托 agent 的标准节奏是"下指令 → 审方案 → 验交付"——你出三次手，循环跑完整件事；VS Code 与 Git 补上「看」和「退路」，三件套才算就位。
...
> 这张图在说什么：上面是没有项目须知的循环——每轮对话手动介绍背景，换个对话全部重来；下面是一次写入、每轮自动注入的机制。写一次，永久生效。
...
3. **撤销**——如果文件夹是 Git 仓库，支持 `/undo` 回退上一轮文件改动（3.6：安全网要在任务之前装好）。
